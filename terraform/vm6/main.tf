terraform {
  ...
  resource "libvirt_volume" "vol-my_vm-1" {
    ...
    resource "libvirt_cloudinit_disk" "vol-cloudinit-my_vm-1" {
  ...
  # We fetch the latest ubuntu release image from their mirrors
  resource "libvirt_volume" "vol-my_vm2-2" {
    name   = "vol-my_vm2-2.qcow2"
    pool   = "default" 
    source = "/pool/iso/ubuntu22.04.img"
    format = "qcow2"
  }
  
    resource "libvirt_cloudinit_disk" "vol-cloudinit-my_vm2-2" {
     name   = "vol-cloudinit-my_vm2-2.qcow2"
     pool  = "default"
     user_data = templatefile("${path.module}/cloud_init.cfg", {
      host_name =  "my_vm2"
      auth_key  = file("~/.ssh/id_vm.pub")
    ...
    network_config = templatefile("${path.module}/network_config.cfg", {
      interface = "ens4"
      ip_addr   = "10.100.14.2"
      mac_addr  = "ae:52:0a:b0:14:02"
    })
  }
  # Create the machine
  resource "libvirt_domain" "dom-my_vm2-2" {
    name       = "my_vm2-2"
    memory     = "4096"
    vcpu       =  "4"
    machine    = "pc"
  ...
  #  qemu_agent = true # this is recommended to get ip address
    disk {
      volume_id = libvirt_volume.vol-my_vm2-2.id
    }
    cloudinit = libvirt_cloudinit_disk.vol-cloudinit-my_vm2-2.id
    network_interface {
      network_name = "default"
    }
    network_interface {
      bridge    = "br0"
      addresses = ["10.100.14.2"]
